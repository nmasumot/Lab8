describe('Basic user flow for SPA ', () => {
  ...
  it('Test5: On first Entry page - checking <entry-page> contents', async () => {
    ...
    const entryCheck = await entry.jsonValue();
    expect(entryCheck).toEqual({ 
      title: 'You like jazz?',
      date: '4/25/2021',
      content: "According to all known laws of aviation, there is no way a bee should be able to fly. Its wings are too small to get its fat little body off the ground. The bee, of course, flies anyway because bees don't care what humans think is impossible.",
      image: {
  ...
  it('Test6: On first Entry page - checking <body> element classes', async () => {
    ...
    const bodyCheck = await page.$eval('body', (e) => e.classList);
    expect(bodyCheck).toEqual({"0": "single-entry"});
  });
  ...
  it('Test9: On Settings page - checking <body> element classes', async () => {
    ...
    const bodyCheck = await page.$eval('body', (e) => e.classList);
    expect(bodyCheck).toEqual({"0": "settings"});
  });
  ...
  it('Test13: On the home page the <body> element should not have any class attribute', async() => {
    const bodyCheck = await page.$eval('body', (e) => e.classList);
    expect(bodyCheck).toEqual({ });
  });

  // define and implement test14: Verify the url is correct when clicking on the second entry
  it('Teste14: When the user clicks on the second entry, the url should have "/#entry2"', async() =>{
    const entries = await page.$$('journal-entry');
  ...
  it('Test15: When the page is on the second entry, the title should be "Entry 2"', async() => {
    const header = await page.$('h1');
    const text = await page.evaluate(element => element.textContent, header);
  ...
  it('Test 16: On second Entry page - checking <entry-page> contents', async() => {
    const entryPage = await page.$('entry-page');
    const entry = await entryPage.getProperty('entry');
    const entryCheck = await entry.jsonValue();
    expect(entryCheck).toEqual({ 
  ...
  it('Test20: Clicking the forward button, new URL should be /#settings', async() => {
